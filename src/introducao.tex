...
O objetivo principal deste projeto é a criação de um aplicativo de edição de foto baseado em nodes, com a sua principal finalidade sendo a implementação do diagrama de fluxo de dados, de uma forma visual, de forma que o usuário possa editar fotos e/ou imagens de uma forma não linear.
...
\begin{itemize}
    ...
    \item Implementar diferentes versões do mesmo aplicativo, para fins de comparação de desempenho e usabilidade, sendo elas: duas versões desktop e uma versão web.
\end{itemize}

\section{Organização dos Capítulos}

Na presente seção será apresentada a organização deste trabalho, dividido em seções. Iniciando pela introdução, onde este capítulo apresenta o tema, a justificativa e os objetivos do trabalho. Em seguida pela fundamentação teórica, que aborda os conceitos e tecnologias relevantes para o desenvolvimento do aplicativo. Posteriormente, são discutidos os trabalhos relacionados, que analisam soluções similares existentes no mercado. O desenvolvimento do trabalho é detalhado na seção de metodologia, criação do aplicativo, que inclui o planejamento e implementação do aplicativo. Finalmente, a conclusão do trabalho apresenta os resultados alcançados, as contribuições do estudo e sugestões para trabalhos futuros.