...
O aplicativo desenvolvido é um editor de imagens baseado em nodes, permitindo aos usuários criar fluxos de edição personalizados conectando visualmente diferentes módulos de processamento. Para fins de comparação, foram implementadas três versões distintas do mesmo aplicativo, cada uma utilizando tecnologias diferentes, a fim de demonstrar as vantagens e limitações de cada abordagem. Todas as versões compartilham a mesma arquitetura e funcionalidades principais, diferenciando-se apenas pela linguagem de programação, bibliotecas utilizadas e desempenho.
...
A versão WebAssembly do aplicativo mantém a mesma estrutura e funcionalidades das versões desktop, feita em C++, adaptando-se ao ambiente do navegador. O aplicativo é compilado para WebAssembly através do Emscripten, permitindo a execução diretamente no navegador com performance próxima ao nativo.
...
A implementação da classe base estabelece padrões nos quais nós subsequentes devem seguir, garantindo consistência e reusabilidade. O Pseudocódigo abaixo apresenta uma visão geral da estrutura e principais métodos da classe base \textit{NodeCore}. 
...
A imagem [\ref{fig:versao-wasm}] ilustra um fluxo completo do sistema, demonstrando a interconexão entre diferentes tipos de nós e o caminho que os dados percorrem desde a entrada até a saída final.
